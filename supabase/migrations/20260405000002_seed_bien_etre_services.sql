-- ═══════════════════════════════════════════════════════════════
-- Migration: Seed Bien Être services
-- 1 category, 3 services, 8 variants
-- ═══════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION add_bien_etre_services(p_salon_id UUID)
RETURNS VOID AS $$
DECLARE
  v_cat_bien_etre  UUID;
  v_svc_hammam     UUID;
  v_svc_massage    UUID;
  v_svc_soin       UUID;
BEGIN
  -- Idempotency guard: skip if 'Hammam Classique' variant already exists
  IF EXISTS (
    SELECT 1
  ...
  -- Find or create Bien Être category
  SELECT id INTO v_cat_bien_etre
  FROM service_categories
  WHERE salon_id = p_salon_id AND name = 'Bien Être' AND deleted_at IS NULL
  LIMIT 1;

  IF v_cat_bien_etre IS NULL THEN
    v_cat_bien_etre := gen_random_uuid();
    INSERT INTO service_categories (id, salon_id, name, color, sort_order) VALUES
      (v_cat_bien_etre, p_salon_id, 'Bien Être', 'bg-teal-100 text-teal-700', 5);
  END IF;

  -- Generate service IDs
  v_svc_hammam  := gen_random_uuid();
  v_svc_massage := gen_random_uuid();
  v_svc_soin    := gen_random_uuid();

  -- Insert 3 services
  INSERT INTO services (id, salon_id, name, category_id, active) VALUES
    (v_svc_hammam,  p_salon_id, 'Hammam',      v_cat_bien_etre, true),
    (v_svc_massage, p_salon_id, 'Massage',      v_cat_bien_etre, true),
    (v_svc_soin,    p_salon_id, 'Soin Visage',  v_cat_bien_etre, true);

  -- Insert 8 variants (durations estimated; costs not provided → 0.00)
  INSERT INTO service_variants (salon_id, service_id, name, duration_minutes, price, cost, additional_cost, sort_order) VALUES
    -- Hammam (3 variantes)
    (p_salon_id, v_svc_hammam,  'Hammam Classique',   60,  120, 0.00, 0.00, 1),
    (p_salon_id, v_svc_hammam,  'Hammam Premium',      90,  160, 0.00, 0.00, 2),
    (p_salon_id, v_svc_hammam,  'Hammam Royal',       120,  250, 0.00, 0.00, 3),
    -- Massage (2 variantes)
    (p_salon_id, v_svc_massage, 'Massage Relaxant 45', 45,  120, 0.00, 0.00, 1),
    (p_salon_id, v_svc_massage, 'Massage Relaxant 60', 60,  200, 0.00, 0.00, 2),
    -- Soin Visage (3 variantes)
    (p_salon_id, v_svc_soin,    'Soin Profond',        60,  200, 0.00, 0.00, 1),
    (p_salon_id, v_svc_soin,    'Soin Éclat',          75,  350, 0.00, 0.00, 2),
    (p_salon_id, v_svc_soin,    'Soin Hydrafacial',    75,  600, 0.00, 0.00, 3);

END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Apply to all existing salons
SELECT add_bien_etre_services(id) FROM salons;

-- ═══════════════════════════════════════════════════════════════
-- Update seed_salon_demo_data to include Bien Être for new salons
-- ═══════════════════════════════════════════════════════════════
...
DECLARE
  ...
  -- Service IDs (Soins only — all other categories come from shared functions)
