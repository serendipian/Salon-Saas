-- ═══════════════════════════════════════════════════════════════
-- Migration: Seed Cils services
-- 1 category, 4 services, 9 variants
-- ═══════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION add_cils_services(p_salon_id UUID)
RETURNS VOID AS $$
DECLARE
  v_cat_cils          UUID;
  v_svc_normaux       UUID;
  v_svc_permanents    UUID;
  v_svc_browlift      UUID;
  v_svc_lashlift      UUID;
BEGIN
  -- Idempotency guard: skip if 'Cils Normaux' service already exists
  IF EXISTS (
    SELECT 1
    FROM services s
  ...
  -- Find or create Cils category
  SELECT id INTO v_cat_cils
  FROM service_categories
  WHERE salon_id = p_salon_id AND name = 'Cils' AND deleted_at IS NULL
  LIMIT 1;

  IF v_cat_cils IS NULL THEN
    v_cat_cils := gen_random_uuid();
    INSERT INTO service_categories (id, salon_id, name, color, sort_order) VALUES
      (v_cat_cils, p_salon_id, 'Cils', 'bg-fuchsia-100 text-fuchsia-700', 6);
  END IF;

  -- Generate service IDs
  v_svc_normaux    := gen_random_uuid();
  v_svc_permanents := gen_random_uuid();
  v_svc_browlift   := gen_random_uuid();
  v_svc_lashlift   := gen_random_uuid();

  -- Insert 4 services
  INSERT INTO services (id, salon_id, name, category_id, active) VALUES
    (v_svc_normaux,    p_salon_id, 'Cils Normaux',    v_cat_cils, true),
    (v_svc_permanents, p_salon_id, 'Cils Permanents', v_cat_cils, true),
    (v_svc_browlift,   p_salon_id, 'Browlift',        v_cat_cils, true),
    (v_svc_lashlift,   p_salon_id, 'Lashlift',        v_cat_cils, true);

  -- Insert 9 variants (durations estimated; costs not provided → 0.00)
  INSERT INTO service_variants (salon_id, service_id, name, duration_minutes, price, cost, additional_cost, sort_order) VALUES
    -- Cils Normaux (1 variante)
    (p_salon_id, v_svc_normaux,    'Cils Normaux',              60,   50, 0.00, 0.00, 1),
    -- Cils Permanents (6 variantes)
    (p_salon_id, v_svc_permanents, 'Classique',                 90,  500, 0.00, 0.00, 1),
    (p_salon_id, v_svc_permanents, '1.5D',                      90,  550, 0.00, 0.00, 2),
    (p_salon_id, v_svc_permanents, '2D',                        100, 600, 0.00, 0.00, 3),
    (p_salon_id, v_svc_permanents, '2.5D',                      110, 650, 0.00, 0.00, 4),
    (p_salon_id, v_svc_permanents, '3D',                        120, 700, 0.00, 0.00, 5),
    (p_salon_id, v_svc_permanents, '3.5D',                      130, 750, 0.00, 0.00, 6),
    -- Browlift (1 variante)
    (p_salon_id, v_svc_browlift,   'Browlift',                  60,  200, 0.00, 0.00, 1),
    -- Lashlift (1 variante)
    (p_salon_id, v_svc_lashlift,   'Lashlift',                  60,  250, 0.00, 0.00, 1);

END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Apply to all existing salons
SELECT add_cils_services(id) FROM salons;

-- ═══════════════════════════════════════════════════════════════
-- Update seed_salon_demo_data to include Cils for new salons
-- ═══════════════════════════════════════════════════════════════
