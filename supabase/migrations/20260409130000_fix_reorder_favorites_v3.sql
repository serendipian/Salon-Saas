-- Fix: use ANY() instead of unnest() to avoid search_path issues with SECURITY DEFINER
...
BEGIN
  ...
  IF NOT (p_salon_id = ANY(user_salon_ids_with_role(ARRAY['owner', 'manager']))) THEN
