...
BEGIN
  ...
  -- Find or create the Coiffure category
  ...
  IF v_cat_coiffure IS NULL THEN
    v_cat_coiffure := gen_random_uuid();
  ...
  INSERT INTO service_variants (salon_id, service_id, name, duration_minutes, price, cost, additional_cost, sort_order) VALUES
    ...
    (p_salon_id, v_svc_brushing, 'Court',         20,  30,    0.00, 0.00, 1),
    (p_salon_id, v_svc_brushing, 'Mi-long',       30,  40,    0.00, 0.00, 2),
    (p_salon_id, v_svc_brushing, 'Long',          35,  50,    0.00, 0.00, 3),
    (p_salon_id, v_svc_brushing, 'Wavy Babyliss', 45,  70,    0.00, 0.00, 4),
    (p_salon_id, v_svc_brushing, 'Wavy Invité',   60,  150,   5.00, 0.00, 5),
    (p_salon_id, v_svc_brushing, 'Plaques',       20,  50,    0.00, 0.00, 6),
    -- Coloration (5 variantes)
    (p_salon_id, v_svc_coloration, 'Racine',                      50,  200, 52.00, 0.00, 1),
    (p_salon_id, v_svc_coloration, 'Normale (à partir de)',       90,  250, 52.00, 0.00, 2),
    (p_salon_id, v_svc_coloration, 'Sans ammoniac (à partir de)', 90,  250, 61.00, 0.00, 3),
    (p_salon_id, v_svc_coloration, 'Balayage (à partir de)',      180, 700, 34.67, 0.00, 4),
    (p_salon_id, v_svc_coloration, 'Ombré (à partir de)',         180, 700, 34.67, 0.00, 5),
    -- Shampoings (4 variantes)
    (p_salon_id, v_svc_shampoing, 'SH Normal',    5, 10,  4.00, 0.00, 1),
    (p_salon_id, v_svc_shampoing, 'Mask Normal',  5, 10,  6.25, 0.00, 2),
    (p_salon_id, v_svc_shampoing, 'SHP Spécial',  5, 20,  8.75, 0.00, 3),
    (p_salon_id, v_svc_shampoing, 'Mask Spécial', 5, 20,  8.75, 0.00, 4),
    -- Coupes (3 variantes)
    (p_salon_id, v_svc_coupes, 'Pointes',   15,  40,  0.00, 0.00, 1),
    (p_salon_id, v_svc_coupes, 'Frange',    15,  40,  0.00, 0.00, 2),
    (p_salon_id, v_svc_coupes, 'Relooking', 35, 120,  0.00, 0.00, 3),
    -- Soins Capillaires (7 variantes)
    (p_salon_id, v_svc_soins, 'Soin K18',                      60, 300, 145.00, 0.00, 1),
    (p_salon_id, v_svc_soins, 'Soin Ker Factor',               60, 300,  66.67, 0.00, 2),
    (p_salon_id, v_svc_soins, 'Soins Antichute (à partir de)', 60, 300, 100.00, 0.00, 3),
    (p_salon_id, v_svc_soins, 'Soins Cadiveau',                60, 400,  90.00, 0.00, 4),
    (p_salon_id, v_svc_soins, 'Soins Aminoplex',               60, 400, 233.33, 0.00, 5),
    (p_salon_id, v_svc_soins, 'Soins Caviar',                  60, 400, 233.33, 0.00, 6),
    (p_salon_id, v_svc_soins, 'Soins Plasma',                  80, 700, 250.00, 0.00, 7),
    -- Lissages Protéines (8 variantes)
    (p_salon_id, v_svc_lissages, 'Cadiveau (à partir de)',              150,  900, 160.00, 0.00, 1),
    (p_salon_id, v_svc_lissages, 'Cadiveau BTX (à partir de)',          150,  900, 160.00, 0.00, 2),
    (p_salon_id, v_svc_lissages, 'Sorali Therapy (à partir de)',        150,  700, 165.00, 0.00, 3),
    (p_salon_id, v_svc_lissages, 'Fit Tanino (à partir de)',            150,  800, 100.00, 0.00, 4),
    (p_salon_id, v_svc_lissages, 'Green (à partir de)',                 150,  900, 140.00, 0.00, 5),
    (p_salon_id, v_svc_lissages, 'Goldery (à partir de)',               150, 1200, 233.33, 0.00, 6),
    (p_salon_id, v_svc_lissages, 'Black Diamond Premium (à partir de)', 150, 1500, 200.00, 0.00, 7),
    (p_salon_id, v_svc_lissages, 'Racine (à partir de)',                120,  500,  80.00, 0.00, 8),
    -- Extensions (5 variantes)
    (p_salon_id, v_svc_extensions, 'Mèche par mèche anneaux (par unité)',  180,   50,  17.00, 0.00, 1),
    (p_salon_id, v_svc_extensions, 'Bandes adhésives premium (par unité)', 180,   70,  25.00, 0.00, 2),
    (p_salon_id, v_svc_extensions, 'Mèche par mèche 6D',                   180,   20,  17.00, 0.00, 3),
    (p_salon_id, v_svc_extensions, 'Extension clips 100g',                  180, 1800, 800.00, 0.00, 4),
    (p_salon_id, v_svc_extensions, 'Extension en kératine (par unité)',     180,   50,  18.00, 0.00, 5),
    -- Coiffures Invitées (5 variantes)
    (p_salon_id, v_svc_invitees, 'Demi chignon (à partir de)',                        90,  150, 2.00, 0.00, 1),
    (p_salon_id, v_svc_invitees, 'Coiffure invitée (à partir de)',                    90,  300, 2.00, 0.00, 2),
    (p_salon_id, v_svc_invitees, 'Babyliss (à partir de)',                            60,  120, 5.00, 0.00, 3),
    (p_salon_id, v_svc_invitees, 'Babyliss Wavy (à partir de)',                       60,  150, 5.00, 0.00, 4),
    (p_salon_id, v_svc_invitees, 'Demi chignon + location extensions (à partir de)', 120,  300, 0.00, 0.00, 5);
...
BEGIN
  ...
  -- ═══════════════════════════════════════════
  -- APPOINTMENTS
  -- Amina  → Brushing (Court variant: 20 min, 30.00)
  -- Sarah  → Coloration (Racine variant: 50 min, 200.00)
  -- Nadia  → Soin Visage complet (unchanged)
  -- Yasmin → Manucure (unchanged)
  -- Fatima → Brushing (Long variant: 35 min, 50.00)
  -- ═══════════════════════════════════════════
