# Onglerie Services Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add the Onglerie category with 7 services and 24 variants to the database, and expose `additional_cost` throughout the frontend.

**Architecture:** Two SQL migrations (schema + data) applied to remote Supabase, followed by manual type updates in `lib/database.types.ts`, then type/mapper/schema/hook updates in the services module. No test framework exists — TypeScript build serves as verification.

**Tech Stack:** PostgreSQL (Supabase), TypeScript, React, TanStack Query, Zod
...
| Action | Path |
|--------|------|
| Create | `supabase/migrations/20260331100000_add_additional_cost_to_variants.sql` |
| Create | `supabase/migrations/20260331100001_seed_onglerie_services.sql` |
| Modify | `lib/database.types.ts` (manual — remote Supabase, `db:types` script uses `--local`) |
| Modify | `types.ts` — add `additionalCost` to `ServiceVariant` |
| Modify | `modules/services/mappers.ts` — map `additional_cost` ↔ `additionalCost` |
| Modify | `modules/services/schemas.ts` — add `additionalCost` to `serviceVariantSchema` |
| Modify | `modules/services/hooks/useServices.ts` — include `additional_cost` in update mutation |

---

## Task 1: Migration — Add `additional_cost` column
...
```sql
-- supabase/migrations/20260331100000_add_additional_cost_to_variants.sql
ALTER TABLE service_variants
  ADD COLUMN additional_cost NUMERIC(10,2) NOT NULL DEFAULT 0;
```

- [ ] **Step 2: Push to remote Supabase**

```bash
supabase db push
```

Expected output: `Applying migration 20260331100000_add_additional_cost_to_variants.sql... done`

- [ ] **Step 3: Commit**

```bash
git add supabase/migrations/20260331100000_add_additional_cost_to_variants.sql
git commit -m "feat: add additional_cost column to service_variants"
```

---

## Task 2: Migration — Insert Onglerie data + update seed

**Files:**
- Create: `supabase/migrations/20260331100001_seed_onglerie_services.sql`
- (This also updates `seed_salon_demo_data` in place via `CREATE OR REPLACE`)

- [ ] **Step 1: Create the migration file**

```sql
-- supabase/migrations/20260331100001_seed_onglerie_services.sql
...
Expected output: `Applying migration 20260331100001_seed_onglerie_services.sql... done`

- [ ] **Step 3: Verify in Supabase dashboard**

Open the Supabase Table Editor → `service_categories`. Confirm your salon has a new `Onglerie` row.
Open `service_variants` and filter by a manucure service_id. Confirm `additional_cost` = 0.

- [ ] **Step 4: Commit**

```bash
git add supabase/migrations/20260331100001_seed_onglerie_services.sql
git commit -m "feat: add Onglerie category with 7 services and 24 variants"
```

---

## Task 3: Update `lib/database.types.ts`

**Files:**
- Modify: `lib/database.types.ts`

> Note: `npm run db:types` uses `--local` and won't work with remote-only Supabase. Update manually — it's just adding `additional_cost` in 3 places.

- [ ] **Step 1: Add `additional_cost` to the `service_variants` Row type**

Find the `service_variants` Row block (around line 1037) and add:
...
- [ ] **Step 2: Add `additional_cost` to the Insert type**
...
- [ ] **Step 3: Add `additional_cost` to the Update type**
...
git commit -m "feat: add additional_cost to service_variants database types"
```
...
- [ ] **Step 1: Add `additionalCost` to `ServiceVariant`**
...
Expected: build fails only on mapper errors (not yet updated) — that's expected and confirms the type is in use.

- [ ] **Step 3: Commit**

```bash
git add types.ts
git commit -m "feat: add additionalCost to ServiceVariant type"
```

---

## Task 5: Update `modules/services/mappers.ts`

**Files:**
- Modify: `modules/services/mappers.ts`

- [ ] **Step 1: Update `toServiceVariant` to map `additional_cost`**
...
- [ ] **Step 2: Add `additional_cost` to `ServiceVariantRow` interface**
...
- [ ] **Step 3: Update `toVariantInsert` to include `additional_cost`**
...
Expected: build fails only on `useServices.ts` update mutation (not yet updated) — confirms remaining work.

- [ ] **Step 5: Commit**

```bash
git add modules/services/mappers.ts
git commit -m "feat: map additional_cost in service variant mappers"
```

---

## Task 6: Update `modules/services/schemas.ts`

**Files:**
- Modify: `modules/services/schemas.ts`

- [ ] **Step 1: Add `additionalCost` to `serviceVariantSchema`**
...
```bash
git add modules/services/schemas.ts
git commit -m "feat: add additionalCost to service variant schema"
```

---

## Task 7: Update `modules/services/hooks/useServices.ts`

**Files:**
- Modify: `modules/services/hooks/useServices.ts`

The update mutation explicitly lists fields when updating an existing variant. Add `additional_cost` to that list.

- [ ] **Step 1: Add `additional_cost` to the variant update call**

Find the section inside `updateServiceMutation` that updates existing variants (around line 122):
...
git commit -m "feat: include additional_cost in service variant update mutation"
```
...
- [ ] **Step 2: Open the app and navigate to Services**

Confirm:
- Category `Onglerie` appears in the sidebar filter
- 7 services are visible under Onglerie
- Clicking `Manucure` shows 3 variants: Simple, Spa, Russe with correct prices (50, 80, 200)
- Clicking `Faux Ongles / Capsules` → OULAC variant: price 80, cost 22.86, additional 8

- [ ] **Step 3: Open a service form and check `additionalCost` field initializes to 0**

(The `ServiceForm` component will need `additionalCost` in its form state — check if the form renders without console errors.)

- [ ] **Step 4: Commit if any minor fixes were needed**

```bash
git add -p
git commit -m "fix: resolve any form init issues for additionalCost"
```
