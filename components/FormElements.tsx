  ...
  return (
    <div className={className} ref={containerRef}>
      ...
      <div className="relative">
        ...
        >
          <span
            className={`truncate ${selectedOption ? 'text-slate-900' : 'text-slate-400'} flex-1`}
          >
            {selectedOption ? (
        ...
        {!isMobile && isOpen && (
          <div
            className="absolute top-[calc(100%+4px)] left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-2xl ring-1 ring-black/5 max-h-80 overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2 duration-200 origin-top"
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden flex-1">
                        {option.image ? (
                          ...
                          <div
                            className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold border shrink-0 shadow-sm transition-colors ${isSelected ? 'bg-white border-slate-300 text-slate-900' : 'bg-slate-100 border-slate-200 text-slate-500'}`}
                          >
                            {option.initials}
                          </div>
                        ) : null}
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                          <div
                            className={`text-sm leading-tight ${isSelected ? 'font-medium' : 'font-normal'}`}
                          >
                            {option.label}
                          </div>
                          {option.subtitle && (
                            <div
                              className={`text-xs truncate mt-0.5 transition-colors font-normal ${isSelected ? 'text-slate-600' : 'text-slate-400 group-hover:text-slate-500'}`}
                            >
                              {option.subtitle}
                            </div>
                          )}
