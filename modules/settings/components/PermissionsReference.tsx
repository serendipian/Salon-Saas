...
const MODULES: ModulePermissions[] = [
  ...
  },
  {
    ...
    access: {
      ...
      stylist:      [{ allowed: true, scope: 'Ses rdv' }, { allowed: false }, { allowed: false }, { allowed: false }],
  ...
  },
  {
    ...
    access: {
      owner:        [{ allowed: true }, { allowed: true }, { allowed: true }, { allowed: true }],
      manager:      [{ allowed: true }, { allowed: true }, { allowed: true }, { allowed: true }],
      stylist:      [{ allowed: true, scope: 'Assignés' }, { allowed: false }, { allowed: false }, { allowed: false }],
      receptionist: [{ allowed: true }, { allowed: true }, { allowed: true }, { allowed: false }],
    },
  },
  {
...
function CellContent({ entry }: { entry: { allowed: boolean; scope?: string } }) {
  ...
  if (entry.scope) return <span className="text-[11px] text-amber-600 font-medium">{entry.scope}</span>;
  ...
  return (
    <>
      {ROLES.map(role => {
        ...
        if (allowed === 0) return <td key={role} className="py-2.5 px-2 text-center"><Minus className="w-3.5 h-3.5 text-slate-300 mx-auto" /></td>;
        if (allowed === total && !hasScope) return <td key={role} className="py-2.5 px-2 text-center"><Check className="w-3.5 h-3.5 text-emerald-500 mx-auto" /></td>;
        return (
          <td key={role} className="py-2.5 px-2 text-center">
            <span className="text-[11px] font-medium text-amber-600">{allowed}/{total}</span>
          </td>
        );
      })}
    </>
...
function ModuleGroup({ mod }: { mod: ModulePermissions }) {
  ...
  return (
    <>
      ...
      {expanded && mod.actions.map((action, i) => (
      ...
      {isOpen && (
        <div className="border-t border-slate-200 p-4 overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-2 pr-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Module</th>
                {ROLES.map(r => (
                  <th key={r} className="py-2 px-2 font-medium text-slate-500 text-center text-xs uppercase tracking-wide">{ROLE_LABELS[r]}</th>
                ))}
              </tr>
            </thead>
            <tbody>
          ...
          <p className="text-[11px] text-slate-400 mt-3 flex items-center gap-3">
            <span className="flex items-center gap-1"><Check className="w-3 h-3 text-emerald-500" /> Accès complet</span>
            <span className="flex items-center gap-1"><Minus className="w-3 h-3 text-slate-300" /> Pas d'accès</span>
