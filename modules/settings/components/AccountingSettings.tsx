              ...
              <div>
                ...
                <div className="mt-3 max-w-xl flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                  ...
                  <p className="text-xs text-blue-700 leading-relaxed">
                    Ce taux unique est appliqué à l'ensemble du chiffre d'affaires pour estimer la
                    TVA à provisionner sur le tableau de bord. Il ne remplace pas votre déclaration
                    officielle — vérifiez les taux applicables avec votre comptable.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="max-w-xl space-y-6 animate-in fade-in">
              ...
              <div className="space-y-2">
                {expenseCategories.map((cat) => (
                  <div
                    key={cat.id}
                    className="flex items-center justify-between p-3 border border-slate-200 rounded-lg group hover:bg-slate-50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-3 h-3 rounded-full ${cat.color.split(' ')[0].replace('bg-', 'bg-')}`}
                      ></div>
