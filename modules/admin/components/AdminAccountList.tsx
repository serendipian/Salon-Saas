...
export const AdminAccountList: React.FC = () => {
  ...
  return (
    <div className="p-8" style={ADMIN_FONT}>
      ...
      {/* Functional search only — no decorative filter chips */}
                      ...
                      >
                        <td className="px-6 py-3 text-[14px]">
                          ...
                          {account.is_suspended && (
                            <span className="ml-2 text-[11px] font-medium px-2.5 py-0.5 rounded-full" style={{ color: '#df1b41', backgroundColor: '#fff0f0' }}>
                              SUSPENDU
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-3 text-[14px]" style={{ color: '#697386' }}>{new Date(account.created_at).toLocaleDateString('fr-FR')}</td>
                        <td className="px-6 py-3 text-right"><ChevronRight size={14} style={{ color: '#c1cfe0' }} /></td>
                      </tr>
                    );
                  })}
                </tbody>
