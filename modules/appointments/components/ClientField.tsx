...
export default function ClientField({
  ...
  if (newClientData) {
    return (
      <div className="mb-4">
        <div className="text-[11px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">Client *</div>
        <div className="bg-slate-950 border border-pink-500 rounded-lg p-3">
          <div className="flex justify-between items-center mb-2.5">
            <span className="text-[11px] text-pink-500 font-semibold uppercase tracking-wider">Nouveau client</span>
            <button type="button" onClick={() => onNewClientChange(null)} className="text-slate-500 hover:text-slate-300 text-sm">✕</button>
          </div>
          ...
          <p className="text-slate-500 text-[10px] mt-2 italic">Le client sera automatiquement ajouté au CRM</p>
        </div>
      ...
      <div className="mb-4">
        ...
        <div className="bg-slate-950 border border-slate-700 rounded-lg px-3.5 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-pink-500 rounded-full flex items-center justify-center text-white text-xs font-semibold">{initials}</div>
            <div>
              <div className="text-slate-200 text-sm font-medium">{selectedClient.firstName} {selectedClient.lastName}</div>
              <div className="text-slate-500 text-[11px]">{selectedClient.phone ?? ''}</div>
            </div>
          </div>
          <button type="button" onClick={onClearClient} className="text-slate-500 hover:text-slate-300 text-sm">✕</button>
        </div>
      </div>
    );
  }
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-700 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
              {filteredClients.map((client) => (
                ...
                >
                  ...
                  <div>
                    <div className="text-slate-200">{client.firstName} {client.lastName}</div>
